On_Player_Tab_Performance_Data:
    ...
    events:
        ...
        after player joins:
        - tablist add uuid:<server.flag[server_tps_display_uuid]> "name:TPS<&co> <server.recent_tps.parse_tag[<[parse_value].round_down_to_precision[1]>].comma_separated>"
        - while <player.if_null[false]>:
            #- narrate "tab item updated"
            - tablist update uuid:<server.flag[server_tps_display_uuid]> "display:TPS<&co> <server.recent_tps.parse_tag[<[parse_value].round_down_to_precision[1]>].comma_separated>"
            - wait 2s
